...
impl<T: Clone + Minimum> State<Self> for Register<T> {
  fn initial() -> Self {
    Self {
      clock: Clock::minimum(),
      value: T::minimum(),
...
impl<T: Clone + Minimum> Joinable<Self> for Register<T> {
...
impl<T: Clone + Minimum> DeltaJoinable<Self> for Register<T> {
  fn delta_join(s: Self, a: &Self, b: &Self) -> Self {
    s.max(a.clone()).max(b.clone())
  }
}

impl<T: Clone + Minimum> GammaJoinable<Self> for Register<T> {
